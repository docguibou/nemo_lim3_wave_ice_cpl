MODULE sbccpl
   ...
   INTEGER, PARAMETER ::   jpr_taiwx  = 43
   INTEGER, PARAMETER ::   jpr_taiwy  = 44
   INTEGER, PARAMETER ::   jpr_icef   = 45
   ...
   INTEGER, PARAMETER ::   jps_fraqsr = 28            ! fraction of solar net radiation absorb
   INTEGER, PARAMETER ::   jps_ficet  = 29   ! total ice fraction  
   INTEGER, PARAMETER ::   jps_icef   = 30   ! ice floe diameter sent back to the wave model ed in the first ocean level
   ...
   ! Send to waves 
   TYPE(FLD_C) ::   sn_snd_ifrac, sn_snd_icef
   ! Received from waves 
   TYPE(FLD_C) ::    sn_rcv_taiwx, sn_rcv_taiwy, sn_rcv_icef
...
CONTAINS
   ...
   SUBROUTINE sbc_cpl_init( k_ice )     
      ...
      NAMELIST/namsbc_cpl/  sn_snd_temp, sn_snd_alb   , sn_snd_thick, sn_snd_crt   , sn_snd_co2,      &
         ...
         &                  sn_rcv_co2 , nn_cplmodel  , ln_usecplmask, sn_rcv_taiwx, sn_rcv_taiwy,    &
         &                  sn_snd_icef, sn_rcv_icef, sn_snd_ifrac
      !!---------------------------------------------------------------------
      !
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_cpl in configuration namelist', lwp )
      ...
      IF( TRIM( sn_rcv_dqnsdt%cldes ) == 'none' .AND. k_ice /= 0 .AND. k_ice /= 4 .AND. nn_components /= jp_iam_sas .AND. ln_wavice == .FALSE. ) &
      ...
      !                                                      ! ------------------------- ! 
      !    add icewave tau GB
      srcv(jpr_taiwx)%clname = 'O_TAIWX'     ! Radiative stress from waves to ice in the u direction
      IF( TRIM(sn_rcv_taiwx%cldes ) == 'coupled' )  THEN
          srcv(jpr_taiwx)%laction = .TRUE.
         cpl_taiwx = .TRUE.
      ENDIF
      !
      srcv(jpr_taiwy)%clname = 'O_TAIWY'     ! Radiative stress from waves to ice in the v direction
      IF( TRIM(sn_rcv_taiwy%cldes ) == 'coupled' )  THEN
          srcv(jpr_taiwy)%laction = .TRUE.
         cpl_taiwy = .TRUE.
      ENDIF
      !
      srcv(jpr_icef)%clname = 'O_ICEF_r'     ! Ice floes max. diameters after wave breaking
      IF( TRIM(sn_rcv_icef%cldes ) == 'coupled' )  THEN
         srcv(jpr_icef)%laction = .TRUE.
         cpl_icef = .TRUE.
      ENDIF 
      !                                                      ! ------------------------------- !
...
! Currently no namelist entry to determine sending of multi-category ice fraction so use the thickness entry for now
      ...
      !                                                      !  Ice floe diameter ! 
      !                                                      ! ------------------------- !
      ssnd(jps_icef)%clname = 'O_ICEF_s'
      IF (TRIM( sn_snd_icef%cldes )   == 'coupled') ssnd(jps_icef)%laction = .TRUE. 
      !
      !                                                      ! ------------------------- !
      !                                                      !      Surface current      !
      !                                                      ! ------------------------- !
      ...
      IF ( ln_wave .AND. nn_components == 0 ) THEN             !XAV 
      ncpl_qsr_freq = 1;
      WRITE(numout,*) 'ncpl_qsr_freq is set to 1 when coupling NEMO with wave (without SAS) '
      ENDIF
      IF ( ln_wavice .AND. nn_components == 0 ) THEN             !GB 
      ncpl_qsr_freq = 1;
      WRITE(numout,*) 'ncpl_qsr_freq is set to 1 when coupling NEMO with wave and ice (without SAS) '
      ENDIF
...
!CDIR NOVERRCHK
      ...
      ENDIF
         !
!received new variable tauice from WW3
        IF (TRIM( sn_rcv_taiwx%clvor ) == 'eastward-northward' ) THEN   ! 2 components oriented along the local grid
               !                                                       ! (geographical to local grid -> rotate the components)
        CALL rot_rep( frcv(jpr_taiwx)%z3(:,:,1), frcv(jpr_taiwx)%z3(:,:,1), srcv(jpr_taiwx)%clgrid, 'en->i', TAIWX)   
        CALL rot_rep( frcv(jpr_taiwx)%z3(:,:,1), frcv(jpr_taiwy)%z3(:,:,1), srcv(jpr_taiwx)%clgrid, 'en->j', TAIWY)   
        ELSE 
         IF( srcv(jpr_taiwx)%laction     ) TAIWX(:,:)    = frcv(jpr_taiwx)%z3(:,:,1)
         IF( srcv(jpr_taiwy)%laction     ) TAIWY(:,:)    = frcv(jpr_taiwy)%z3(:,:,1)
        ! Should be nicely coded later
        ENDIF 
        ! The conversion from T point to U,V grids is made in limrhg so far...  
      IF( srcv(jpr_icef)%laction      ) ICEF_r(:,:)   = frcv(jpr_icef)%z3(:,:,1)
...
!                  ztmp_ai=a_i(ji,jj,1)
!                  DO jl=2,jpl
!                   IF (a_i(ji,jj,jl).GE.ztmp_ai) THEN
!                    ztmp_ai      = a_i(ji,jj,jl)
!                    ztmp3(ji,jj,1) = ht_i(ji,jj,jl) 
!                    ztmp4(ji,jj,1) = ht_s(ji,jj,jl)
!                   ENDIF 
!                 ENDDO
!                ENDDO
!              ENDDO
         CASE( 'contour ice and snow'         )   
              WHERE( SUM( a_i, dim=3 ) >= 0.15 )
                  ztmp3(:,:,1) = SUM( ht_i * a_i, dim=3 ) / SUM( a_i, dim=3 )
                  ztmp4(:,:,1) = SUM( ht_s * a_i, dim=3 ) / SUM( a_i, dim=3 )
               ELSEWHERE
                 ztmp3(:,:,1) = 0.
      ...
      END IF 
      IF( ssnd(jps_icef)%laction ) THEN 
         ICEF_s=dmaxfl_i
         CALL cpl_snd( jps_icef, isec, RESHAPE ( ICEF_s(:,:), (/jpi,jpj,1/) ), info ) 
      END IF 
      !
