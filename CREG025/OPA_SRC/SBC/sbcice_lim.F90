...
!!gm   remark, the ocean-ice stress is not saved in ice diag call above .....  find a solution!!!
   ...
   SUBROUTINE ice_run
      ...
      NAMELIST/namicerun/ jpl,jpf, nlay_i, nlay_s, rn_amax_n, rn_amax_s, cn_icerst_in, cn_icerst_indir,   &
         &                cn_icerst_out, cn_icerst_outdir, ln_limthd, ln_limdyn, nn_limdyn, nn_limfldis,  &
         &                rn_flq
      NAMELIST/namicediag/ ln_limdiachk, ln_limdiahsb, ln_icectl, iiceprt, jiceprt  
      !!-------------------------------------------------------------------
      !                    
...
!        Allocate values
         ...
         DO jf = 1, jpf-1                
            ...
            ! di_mean(jf)  =  zgamma/(zgamma-1.) * ( (di_max(jf)**(-zgamma+1.)-di_max(jf-1)**(-zgamma+1.))  &
            ! &               /(di_max(jf)**(-zgamma)-di_max(jf-1)**(-zgamma)) )
         END DO
      CASE (2)
...
!        Allocate values
      ...
      DO jf = 1, jpf-1                
         di_mean(jf)  = ( di_max(jf)+di_max(jf-1) ) /2._wp  
      END DO
      ...
      IF(lwp) WRITE(numout,*) '         di_mean', di_mean(1:jpf) 
      IF(lwp) WRITE(numout,*) '         di_frac', di_frac(1:jpf) 
      !
