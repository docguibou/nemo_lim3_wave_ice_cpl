...
CONTAINS
   ...
   SUBROUTINE lim_rhg( k_j1, k_jpj )
      ...
      REAL(wp), POINTER, DIMENSION(:,:) ::   zTauU_iw , ztauV_iw             ! ice-wave. stress at U-V points
      ...
      REAL(wp), POINTER, DIMENSION(:,:) ::   zdamage                         ! "damage" coefficient   
      ...
      CALL wrk_alloc( jpi,jpj, zaU, zaV, zmU_t, zmV_t, zmf, zTauU_ia, ztauV_ia , zTauU_iw, ztauV_iw)
      CALL wrk_alloc( jpi,jpj, zspgU, zspgV, v_oceU, u_oceV, v_iceU, u_iceV, zfU, zfV )
      CALL wrk_alloc( jpi,jpj, zds, zs1, zs2, zs12, zu_ice, zv_ice, zresr, zpice, zdamage)
...
# endif
      ...
      DO jj = k_j1+1, k_jpj-1
         DO ji = fs_2, fs_jpim1
            ...
            ! Drag ice-wave.
            zTauU_iw(ji,jj) =  rau0 * 0.5 * ( TAIWX(ji+1,jj) + TAIWX(ji,jj) )  * umask(ji,jj,1)
            zTauV_iw(ji,jj) =  rau0 * 0.5 * ( TAIWY(ji,jj+1) + TAIWY(ji,jj) )  * vmask(ji,jj,1)
            !WRITE(*,*) "TAIWX(ji,jj)" , rau0*TAIWX(ji,jj), zTauU_iw(ji,jj),zTauV_iw(ji,jj)
            ...
            ! damage
            IF (ln_icestr_fl) THEN
              IF ( dmaxfl_i(ji,jj) * rn_icestr_c1 < rn_dmin ) THEN
                zdamage(ji,jj) = 1._wp - TANH(rn_dmin - dmaxfl_i(ji,jj) * rn_icestr_c1 / rn_dmin * rn_icestr_c2)
              ELSE
                zdamage(ji,jj) = 1._wp
              END IF
            ELSE
         ...
         DO jj = k_j1, k_jpj-1         ! loops start at 1 since there is no boundary condition (lbc_lnk) at i=1 and j=1 for F points
            DO ji = 1, jpim1
               ...
               zds(ji,jj) = ( ( u_ice(ji,jj+1) * r1_e1u(ji,jj+1) - u_ice(ji,jj) * r1_e1u(ji,jj) ) * e1f(ji,jj) * e1f(ji,jj)   &
                  ...
                  &         ) * r1_e12f(ji,jj) * zfmask(ji,jj) * zdamage(ji,jj)
            END DO
         END DO
         ...
         DO jj = k_j1+1, k_jpj-1
            DO ji = 2, jpim1 ! no vector loop
               ...
               zds2 = ( zds(ji,jj  ) * zds(ji,jj  ) * e12f(ji,jj  ) + zds(ji-1,jj  ) * zds(ji-1,jj  ) * e12f(ji-1,jj  )  &
                  ...
                  &   ) * 0.25_wp * r1_e12t(ji,jj) * zdamage(ji,jj)
         ...
         IF( MOD(jter,2) .EQ. 0 ) THEN ! even iterations
            ...
            DO jj = k_j1+1, k_jpj-1
               DO ji = fs_2, fs_jpim1
                  ...
                  zTauE = zfV(ji,jj) + zTauV_ia(ji,jj) +  zTauV_iw(ji,jj) + zCor + zspgV(ji,jj) + zTauO * ( v_oce(ji,jj) - v_ice(ji,jj) )
            ...
            DO jj = k_j1+1, k_jpj-1
               DO ji = fs_2, fs_jpim1
                  ...
                  ! Sum of external forces (explicit solution) = F + tau_ia + Coriolis + spg + tau_io
                  zTauE = zfU(ji,jj) + zTauU_ia(ji,jj) + zTauU_iw(ji,jj) + zCor + zspgU(ji,jj) + zTauO * ( u_oce(ji,jj) - u_ice(ji,jj) )
      ...
      CALL iom_put( 'stress1_i' , stress1_i  )
      CALL iom_put( 'stress2_i',  stress2_i  )
      CALL iom_put( 'stress12_i', stress12_i  )
      ...
      CALL wrk_dealloc( jpi,jpj, zaU, zaV, zmU_t, zmV_t, zmf, zTauU_ia, ztauV_ia, zTauU_iw, zTauV_iw )
      CALL wrk_dealloc( jpi,jpj, zspgU, zspgV, v_oceU, u_oceV, v_iceU, u_iceV, zfU, zfV )
      CALL wrk_dealloc( jpi,jpj, zds, zs1, zs2, zs12, zu_ice, zv_ice, zresr, zpice, zdamage )
