MODULE limvar
   ...
   USE sbc_ice , ONLY: ICEF_r , ICEF_s       ! Surface boundary condition: ocean fields
   ...
   !! $Id: limvar.F90 6963 2016-09-30 12:40:04Z clem $
...
CONTAINS
   ...
   SUBROUTINE lim_var_agg( kn )
      ...
      IF (numit == nit000 - 1)  THEN
       ...
       DO jj = 1, jpj
          DO ji = 1, jpi
             afl_i(ji,jj,jpf) = MAX(at_i(ji,jj), 0._wp)   ! ice cat. floe fraction

             IF (at_i(ji,jj)>1) WRITE(*,*) 'PB COMMENCE ICI', at_i(ji,jj),afl_i(ji,jj,jpf)
          END DO
       END DO
      END IF
   ...
   SUBROUTINE lim_var_glo2eqv
      ...
      LOGICAL  ::   z_isdiam
      REAL(wp), POINTER, DIMENSION(:,:)      ::   zaiflt  
      !!------------------------------------------------------------------
      CALL wrk_alloc( jpi,jpj,zaiflt )
      !-------------------------------------------------------
      ...
      at_i (:,:) = SUM( a_i, dim=3 )
      ! Ice floes cat.
      zaiflt(:,:) = SUM( afl_i(:,:,:), dim=3 )
!    
      DO jf=1, jpf 
        DO jj = 1, jpj
           DO ji = 1, jpi
              IF (zaiflt(ji,jj)/=at_i(ji,jj)) THEN 
                rswitch         = MAX( 0._wp, SIGN( 1._wp, at_i(ji,jj) - epsi20 ) )
                rswitch         = MAX( 0._wp, SIGN( 1._wp, zaiflt(ji,jj) - epsi20 ) ) * rswitch      
                afl_i(ji,jj,jf) = afl_i(ji,jj,jf)*at_i(ji,jj)/ MAX(zaiflt(ji,jj),epsi10) *rswitch
                IF (zaiflt(ji,jj)==0._wp) THEN
                   afl_i(ji,jj,jpf)=at_i(ji,jj)
                END IF
              END IF
           END DO
        END DO
...
!DMAX
      DO ji = 1 , jpi 
         DO jj = 1 , jpj
            rswitch       = MAX( 0._wp , SIGN( 1._wp, at_i(ji,jj ) - epsi10 ) )
            dmaxfl_i(ji,jj)=rn_fsdmax * rswitch
            IF (afl_i(ji,jj,jpf)<0.5 * at_i(ji,jj)) THEN
              DO jf=jpf-1, 1, -1    
                  IF ( SUM(afl_i(ji,jj,jf:jpf)) >= di_frac(jf) *rn_catfthresh * at_i(ji,jj) ) THEN ! 1st criterion
                    dmaxfl_i(ji,jj)=di_max(jf)   * rswitch
                    EXIT
                  ENDIF
              ENDDO
            END IF
...
!
!            DO jf=jpf, 1, -1    
!                IF (   afl_i(ji,jj,jf) > di_frac(jf) * rn_catfthresh * at_i(ji,jj) ) THEN ! 1st criterion
!                  dmaxfl_i(ji,jj)=di_max(jf)   * rswitch
!                  z_isdiam      = .TRUE.
!                  EXIT
!                ENDIF
!            ENDDO
!            
!            IF ( (at_i(ji,jj)>0).AND.(.NOT.(z_isdiam) ) ) WRITE(*,*) 'DMAX PAS ATTRIBUE', dmaxfl_i(ji,jj),  at_i(ji,jj) 
!  
!            IF (.NOT.(z_isdiam)) THEN
!              DO jf=jpf, 1, -1    
!                  afl_i(ji,jj,jf) = afl_i  (ji,jj,jf) * rswitch
!                  IF (   afl_i(ji,jj,jf) >  at_i(ji,jj) / jpf ) THEN ! 2nd criterion
!                    dmaxfl_i(ji,jj)=di_max(jf)   * rswitch
!                    EXIT          
!                  ENDIF
!              ENDDO
...
!      END DO
      ...
      DO jj = 1, jpj
        DO ji = 1, jpi
          IF ( ABS( zaiflt(ji,jj)-at_i(ji,jj) ) > epsi06 ) WRITE(*,*) 'NOT CONSERVED 3', ' i :', ji, 'j :', jj, 'diff ;', ABS( zaiflt(ji,jj)-at_i(ji,jj) ), "aiflt :", zaiflt(ji,jj),"at_i :", at_i(ji,jj)
        END DO
      END DO
      ICEF_s=dmaxfl_i 
      CALL wrk_dealloc( jpi,jpj,zaiflt )
   ...
   SUBROUTINE lim_var_zapsmall
      ...
      !!-------------------------------------------------------------------
      INTEGER  ::   ji, jj, jl, jk, jf   ! dummy loop indices
...
! Floes cat.
      DO ji = 1 , jpi 
         DO jj = 1 , jpj
            rswitch       = MAX( 0._wp , SIGN( 1._wp, at_i(ji,jj ) - epsi10 ) )
            dmaxfl_i(ji,jj) =  dmaxfl_i(ji,jj) * rswitch
            DO jf = 1 , jpf
               afl_i(ji,jj,jf) = afl_i  (ji,jj,jf) * rswitch
            END DO
         END DO
   ...
   SUBROUTINE lim_var_itd( zhti, zhts, zai, zht_i, zht_s, za_i )
    ...
    SUBROUTINE lim_var_fsd
      !!------------------------------------------------------------------
      !!                ***  ROUTINE lim_var_fsd   ***
      !!
      !! ** Purpose :  Redistribute the floe size distribution after wave breaking
      !!
      !!                  
      !!
      !! ** Method:   Iterative procedure
      ...
      !!
      !!-------------------------------------------------------------------
      !! History : LIM3.X - 2017    (G. Boutin)  Original code
      !!-------------------------------------------------------------------
      !! Local variables
      INTEGER  :: ji, jj, jf, jff, jfn, jfm              ! dummy loop indices
      REAL(wp), POINTER, DIMENSION(:,:)      ::   zadis_array, zaold
      REAL(wp), POINTER, DIMENSION(:,:)      ::   zaiflt,zaiflt_or
      REAL(wp), POINTER, DIMENSION(:,:)      ::   zbeta_array
      REAL(wp), POINTER, DIMENSION(:)        ::   zQ_array
      REAL(wp), POINTER, DIMENSION(:,:,:)    ::   zafl_old 
      LOGICAL, DIMENSION(jpf)      ::   zmask
      INTEGER  :: i_break 
      REAL(wp) :: zbeta, zadis, zfrag, zgamma, zdmin, zdelta
      !REAL(wp), DIMENSION(:),   INTENT(in)    ::   zhti, zhts, zai    ! input ice/snow variables
      !REAL(wp), DIMENSION(:,:), INTENT(inout) ::   zht_i, zht_s, za_i ! output ice/snow variables
      !INTEGER , POINTER, DIMENSION(:)         ::   itest
 
      CALL wrk_alloc( jpi, jpj, zadis_array, zaold )
      CALL wrk_alloc( jpi, jpj, zaiflt,zaiflt_or )
      CALL wrk_alloc( jpf, jpf, zbeta_array )
      CALL wrk_alloc( jpf, zQ_array )
      CALL wrk_alloc( jpi, jpj, jpf, zafl_old )
      ...
      !--------------------------------------------------------------------
       ...
       zgamma           = 2 + log(zfrag)/log(2.)    ! namelist parameter ?
       zdmin            = rn_fsdmin                   ! namelist parameter ?
       zadis_array(:,:) = 0._wp
       zadis            = 0._wp
       i_break          = 1
       zaold            = 0._wp
      ...
      !--------------------------------------------------------------------
      ! Local variables needed, to be cleaned
      !--------------------------------------------------------------------
      ...
      zaiflt(:,:)= SUM( afl_i(:,:,:), dim=3 )
      zaiflt_or(:,:)= zaiflt(:,:)
      zafl_old(ji,jj,jf) = SUM( afl_i(ji,jj,:))
      DO ji = 1, jpi 
        DO jj = 1, jpj

           zadis=0._wp
           zmask=.TRUE.
           IF ((at_i(ji,jj)>0._wp) .AND.(ICEF_r(ji,jj)>0._wp)) dmaxfl_i(ji,jj)=ICEF_r(ji,jj)
           DO jf = 1, jpf-1
             
             IF ( ( dmaxfl_i(ji,jj)< di_max(jf) ).AND.(at_i(ji,jj)>0._wp) ) THEN ! IF Dmax from ww3 lower than di_max associated with the cat. jf
                    i_break=jf
                    dmaxfl_i(ji,jj)=di_max(i_break) ! This is the new dmax
                 IF (nn_limfldis<2) THEN
                     DO jff = i_break+1, jpf              
                        zadis= zadis + afl_i(ji,jj,jff)! Get the fraction occupied by the categories for which d>dmax
                        afl_i(ji,jj,jff) = 0._wp       ! Empty the categories for which d>dmax
                        zmask(jff)=.FALSE.            
                     ENDDO

                     zadis_array(ji,jj)  = zadis                    ! ( broken area / number of categories to redistribute) 
                     zaold(ji,jj) = SUM( afl_i(ji,jj,:))

                     DO jff = 1, i_break

                        IF (nn_limfldis==0) THEN 
                          afl_i(ji,jj,jff) = afl_i(ji,jj,jff) +  zadis / i_break   ! uniform repartition
                        ELSE IF (nn_limfldis==1) THEN ! only the floe cat. D>Dmax are redistributed
                           IF ( jff>1 ) THEN
                              zbeta = ( di_max(jff)**(-zgamma+2) - di_max(jff-1)**(-zgamma+2) ) / ( di_max(i_break)**(-zgamma+2) - zdmin**(-zgamma+2) ) 
                           ELSE
                              zbeta = ( di_max(1)**(-zgamma+2) - zdmin**(-zgamma+2) ) / ( di_max(i_break)**(-zgamma+2) - zdmin**(-zgamma+2) )    
                           ENDIF
                          afl_i(ji,jj,jff) = afl_i(ji,jj,jff) + zadis *zbeta
                        ENDIF
                     ENDDO
                     EXIT ! DO jf = 1, jpf-1

                 ELSE IF (nn_limfldis==2) THEN
                     ...
                     DO jfn = jpf,1,-1
                       zdelta =  - zafl_old(ji,jj,jfn)+( di_max(jfn)**(-zgamma+2) - di_max(jfn-1)**(-zgamma+2) ) / ( di_max(i_break)**(-zgamma+2) - zdmin**(-zgamma+2) ) * zaold(ji,jj)
                       DO jfm = 1, jpf
                        IF ( (jfm >= jfn).AND.(jfn<=i_break) ) THEN
                          zbeta_array(jfm,jfn) = ( di_max(jfn)**(-zgamma+2) - di_max(jfn-1)**(-zgamma+2) ) / ( MIN(di_max(i_break),di_max(jfm))**(-zgamma+2) - zdmin**(-zgamma+2) )
                        END IF
                       END DO 
                       IF (jfn>i_break) THEN
                          zQ_array(jfn)=1._wp
                       ELSE
                          IF ((zafl_old(ji,jj,jfn)>0._wp).AND.(jfn>1)) THEN
                             IF (rn_flq<0._wp) THEN
                               zQ_array(jfn)=MAX((zdelta-SUM( zbeta_array(:,jfn)*zafl_old(ji,jj,:)*zQ_array(:))) / zafl_old(ji,jj,jfn)*(zbeta_array(jfn,jfn)-1._wp),0._wp)
                             ELSE
                               zQ_array(jfn)=rn_flq
                             END IF
                          END IF
                       END IF
                       afl_i(ji,jj,jfn) = zafl_old(ji,jj,jfn) * (1 -zQ_array(jfn)) + SUM(zbeta_array(:,jfn)*zafl_old(ji,jj,:)*zQ_array(:))  
      ...
      DO jj = 1, jpj
        DO ji = 1, jpi
          IF ( ABS( zaiflt(ji,jj)-at_i(ji,jj) ) > epsi06 ) WRITE(*,*) 'FSD CONSERVATION', 'i :', ji, 'j :', jj, 'diff ;', ABS( zaiflt(ji,jj)-at_i(ji,jj) ), "aifl_old", zaold(ji,jj),'aiflt :',zaiflt(ji,jj) ,'ait :',at_i(ji,jj) ,'vecteur zafl_old :',zafl_old(ji,jj,:),'vecteur zafl_new :' ,afl_i(ji,jj,:),'d', dmaxfl_i(ji,jj),ICEF_r(ji,jj)
          !IF (( ABS( (zaold(ji,jj) + zadis_array(ji,jj) ) -at_i(ji,jj) ) > epsi06 ).AND.dmaxfl_i(ji,jj)< di_max(jpf-1) ) WRITE(*,*) 'OLD + DIS', zaold(ji,jj) + zadis_array(ji,jj),'a_i :', at_i(ji,jj),'kept :', zaold(ji,jj) ,'disp :' ,zadis_array(ji,jj),'aifl origi.',zaiflt_or(ji,jj),'aiflt :',zaiflt(ji,jj) ,'aifl :',afl_i(ji,jj,:) , 'd', dmaxfl_i(ji,jj), 'i', ji, 'j', jj
        END DO
      END DO

      CALL wrk_dealloc( jpi, jpj, zadis_array, zaold  )
      CALL wrk_dealloc( jpi, jpj, zaiflt,zaiflt_or )
      CALL wrk_dealloc( jpf, jpf, zbeta_array )
      CALL wrk_dealloc( jpf, zQ_array )
      CALL wrk_dealloc( jpi, jpj, jpf, zafl_old)
      !
    END SUBROUTINE lim_var_fsd
#else
   !!----------------------------------------------------------------------
