...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: limtrp.F90 6476 2016-04-15 12:50:29Z mcastril $
...
CONTAINS
   ...
   SUBROUTINE lim_trp( kt ) 
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:)    ::   zviold, zvsold, zsmvold, zaiflold  ! old ice volume...
      REAL(wp), POINTER, DIMENSION(:,:,:)    ::   zhimax                   ! old ice thickness
      REAL(wp), POINTER, DIMENSION(:,:)      ::   zatold, zeiold, zesold, zaiflt     ! old concentration, enthalpies
      ...
      CALL wrk_alloc( jpi,jpj,            zsm, zatold, zeiold, zesold, zaiflt )
      ...
      IF( ln_limdyn ) THEN          !   Advection of sea ice properties   !
         ...
         DO jj = 1, jpj
            DO ji = 1, jpi
               IF ( ABS( zaiflt(ji,jj)-at_i(ji,jj) ) > epsi06 ) WRITE(*,*) 'CONC. NOT CONSERVED 1 ', 'i :', ji, 'j :', jj, 't :' , kt, 'diff ;', ABS( zaiflt(ji,jj)-at_i(ji,jj) ), zaiflt(ji,jj), at_i(ji,jj)
            END DO
         END DO
         !=============================!
         !==      Prather scheme     ==!
...
!!         ENDIF
         ...
         END DO
         DO jf = 1, jpf
            z0afl(:,:,jf)   = afl_i (:,:,jf) * e12t(:,:)    ! Floe content
         END DO
         ...
         IF( MOD( ( kt - 1) / nn_fsbc , 2 ) == 0 ) THEN       !==  odd ice time step:  adv_x then adv_y  ==!
            DO jt = 1, initad
               ...
               END DO
               DO jf = 1, jpf
                  CALL lim_adv_x( zusnit, u_ice, 1._wp, zsm, z0afl  (:,:,jf), sxfl(:,:,jf),   &    !--- ice floes      ---     
                     &                                       sxxfl(:,:,jf), syfl(:,:,jf), syyfl(:,:,jf), sxyfl(:,:,jf)  )
                  CALL lim_adv_y( zusnit, v_ice, 0._wp, zsm, z0afl  (:,:,jf), sxfl(:,:,jf),   &
                     &                                       sxxfl(:,:,jf), syfl(:,:,jf), syyfl(:,:,jf), sxyfl(:,:,jf)  )
               END DO
            END DO
         ELSE
            DO jt = 1, initad
               ...
               END DO
               DO jf = 1, jpf
                  CALL lim_adv_y( zusnit, v_ice, 1._wp, zsm, z0afl  (:,:,jf), sxfl(:,:,jf),   &   !--- ice floes      ---
                     &                                       sxxfl (:,:,jf), syfl(:,:,jf), syyfl(:,:,jf), sxyfl(:,:,jf)  )
                  CALL lim_adv_x( zusnit, u_ice, 0._wp, zsm, z0afl  (:,:,jf), sxfl(:,:,jf),   &
                     &                                       sxxfl(:,:,jf), syfl(:,:,jf), syyfl(:,:,jf), sxyfl(:,:,jf)  )
               END DO
            END DO
         ENDIF
...
!!gm & cr 
         ...
         DO jl = 1, jpl
            DO jj = 1, jpj
               DO ji = 1, jpi
                  IF ( a_i(ji,jj,jl) > 1._wp ) WRITE(*,*) "HELP HELP HELP",a_i(ji,jj,jl),v_i(ji,jj,jl)
      ...
      DO jf = 1, jpf
        DO jj = 1, jpj
         ...
         END DO
      END DO
      ! --- In case (at_i) has increase from 0, add (afl_i) -----------------
      DO jj = 1, jpj
        DO ji = 1, jpi
            IF ( ( ABS( zaiflt(ji,jj)-at_i(ji,jj) ) > epsi06).AND.(zaiflt(ji,jj)==0) )   afl_i(ji,jj,1) = at_i(ji,jj)
        END DO
      END DO
     ...
     ! TEST GB 2
      ...
      DO jj = 1, jpj
        DO ji = 1, jpi
          IF ( ABS( zaiflt(ji,jj)-at_i(ji,jj) ) > epsi06 ) WRITE(*,*) 'NOT CONSERVED 2 ', 'diff ;', ABS( zaiflt(ji,jj)-at_i(ji,jj) ),  'zaiflt ;',zaiflt(ji,jj), 'at_i ;',at_i(ji,jj)
        END DO
      END DO
      ...
      !
      CALL wrk_dealloc( jpi,jpj,            zsm, zatold, zeiold, zesold, zaiflt )
