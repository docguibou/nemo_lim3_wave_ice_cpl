...
CONTAINS
   ...
   SUBROUTINE lim_wri( kindic )
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:) ::  zswi3
      ...
      CALL wrk_alloc( jpi, jpj, jpf, zswi3 )
      ...
      DO jf = 1, jpf
         DO jj = 1, jpj
            DO ji = 1, jpi
               zswi3(ji,jj,jf)  = MAX( 0._wp , SIGN( 1._wp , afl_i(ji,jj,jf) - epsi06 ) )
            END DO
         END DO
      END DO
      !
      ...
      CALL iom_put( "utau_wice"   , rau0 *TAIWX         )        ! waves stress over ice along i-axis at I-point
      CALL iom_put( "vtau_wice"   , rau0 *TAIWY         )        ! waves stress over ice along j-axis at I-point
      CALL iom_put( "snowpre"     , sprecip * 86400.    )        ! snow precipitation 
      CALL iom_put( "micesalt"    , smt_i   * zswi      )        ! mean ice salinity
      CALL iom_put( "icef_s"      , ICEF_s              )        ! to remove, just for debug
      CALL iom_put( "icef_r"      , ICEF_r              )        ! to remove, just for debug
      CALL iom_put( "icefldmax"   , dmaxfl_i * zswi     )        ! to remove
...
!CT lateral melting add }
      ...
      CALL iom_put ('hfxlam'     , hfx_lam(:,:)         )   !  
      CALL iom_put ('hfxbog'     , hfx_bog(:,:)         )   !  
      CALL iom_put ('hfxdif'     , hfx_dif(:,:)         )   ! 
      CALL iom_put ('hfxcon'     , hfx_con(:,:)         )   !   
      ...
      ! Floe size cat.
      IF ( iom_use( "icefloes_cat" ) )  CALL iom_put( "icefloes_cat"     , afl_i * zswi3   )        ! area for floes categories (GB) (Has to be put in history file too ?)
      ! ice temperature
      ...
      CALL wrk_dealloc( jpi, jpj, jpf, zswi3 )
