...
CONTAINS
   ...
   SUBROUTINE lim_update2( kt )
      ...
      !-----------------------------------------------------
      ! ice concentration in floes cat. must be equal to ice concentration for thickness cat. 
      !-----------------------------------------------------

       DO jj = 1, jpj
          DO ji = 1, jpi
             IF( at_i(ji,jj) > 0 .AND. SUM(afl_i(ji,jj,:)) == 0._wp ) THEN
                afl_i (ji,jj,jpf) = at_i (ji,jj)
                dmaxfl_i(ji,jj)=di_max(1)  
             ENDIF
          END DO
       END DO
