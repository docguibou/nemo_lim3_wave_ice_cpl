MODULE ice
   ...
   INTEGER , PUBLIC ::   nn_catfbnd       !: categories distribution following for floes : to be changed (GB) 
   REAL(wp), PUBLIC ::   rn_catfthresh    !: coefficient for local dmax determination 
   REAL(wp), PUBLIC ::   rn_fsdmax       !: maximum floe size diameter in the model
   REAL(wp), PUBLIC ::   rn_fsdmin       !: minimum floe size diameter in the model
   REAL(wp), PUBLIC ::   rn_fsdwidth       !: minimum floe size diameter in the model
   REAL(wp), PUBLIC ::   rn_maxpareto      !: minimum floe size diameter in the model
...
!CT lateral melting add {
   ...
   INTEGER , PUBLIC ::   nn_lamp          !: choice of lateral melting param : 0 (Lupkes), 1 (use FSD, should only be used if coupled with waves)
!CT lateral melting add }

   !                                     !!** ice-dynamics namelist (namicedyn) **
   LOGICAL , PUBLIC ::   ln_icestr_bvf    !: use brine volume to diminish ice strength
   LOGICAL , PUBLIC ::   ln_icestr_fl     !: use floe size to diminish ice strength
   REAL(wp) , PUBLIC ::  rn_icestr_c1     !: use floe size to diminish ice strength parameter 1
   REAL(wp) , PUBLIC ::  rn_icestr_c2     !: use floe size to diminish ice strength parameter 2
...
!CT lateral melting add
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   hfx_con     !: total heat flux associated with ocean-ice conduction   [W.m-2]
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   afl_i       !: Ice fractional areas for floes (concentration)
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   dmaxfl_i     !: Max. floe diameters (m)
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::   sxfl , syfl , sxxfl , syyfl , sxyfl    !: ice floe
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::   a_i_b, smv_i_b, oa_i_b, afl_i_b     !:
   ...
   INTEGER           , PUBLIC ::   jpf             !: number of ice  floes categories 
   ...
   INTEGER           , PUBLIC ::   nn_limfldis     !: switch for floes redistribution
   REAL              , PUBLIC ::   rn_flq            !: coeff. for floes redistribution
...
!! * Floe size distribution variables
   !!--------------------------------------------------------------------------
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:)   ::   di_max         !: Boundary of floe size categories in floe diameters space
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:)   ::   di_mean        !: Mean floe size in categories 
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:)   ::   di_frac        !: fraction occupied by cat jf supposing that di_max(jf)= dmax
   !
   !!--------------------------------------------------------------------------
   !! * Ice diagnostics
   !!--------------------------------------------------------------------------
...
!CT lateral melting add }
         ...
         &      hfx_thd(jpi,jpj) , hfx_dyn(jpi,jpj) , hfx_spr(jpi,jpj) , hfx_con(jpi,jpj) , hfx_lam(jpi,jpj) ,   &               
         & STAT=ierr(ii) )
      ...
      ALLOCATE( ht_i (jpi,jpj,jpl) , a_i  (jpi,jpj,jpl) , v_i  (jpi,jpj,jpl) ,     &
         ...
         &      oa_i (jpi,jpj,jpl) , bv_i (jpi,jpj,jpl) , afl_i  (jpi,jpj,jpf),  STAT=ierr(ii) )
      ii = ii + 1
      ALLOCATE( u_ice(jpi,jpj) , v_ice(jpi,jpj) ,                                       &
         ...
         &      om_i (jpi,jpj) , tau_icebfr(jpi,jpj)   ,dmaxfl_i(jpi,jpj) , STAT=ierr(ii) )
      ii = ii + 1
      ...
      ALLOCATE( sxa  (jpi,jpj,jpl) , sya  (jpi,jpj,jpl) , sxxa  (jpi,jpj,jpl) , syya  (jpi,jpj,jpl) , sxya  (jpi,jpj,jpl) ,   &
         ...
         &      STAT=ierr(ii) )
      ii = ii + 1
      ALLOCATE( sxfl  (jpi,jpj,jpf) , syfl  (jpi,jpj,jpf) , sxxfl  (jpi,jpj,jpf) , syyfl  (jpi,jpj,jpf) , sxyfl  (jpi,jpj,jpf) ,  &
         &      STAT=ierr(ii) )
      ii = ii + 1
      ...
      ALLOCATE( v_s_b  (jpi,jpj,jpl) , v_i_b  (jpi,jpj,jpl) , e_s_b(jpi,jpj,nlay_s,jpl) ,     &
         ...
         &      oa_i_b (jpi,jpj,jpl) , afl_i_b  (jpi,jpj,jpf)                           , STAT=ierr(ii) )
      ii = ii + 1
      ...
      ! * Ice floes distribution variables
      WRITE(*,*)
      ii = ii + 1
      ALLOCATE( di_max(0:jpf), di_mean(jpf), di_frac(jpf), STAT=ierr(ii) )
