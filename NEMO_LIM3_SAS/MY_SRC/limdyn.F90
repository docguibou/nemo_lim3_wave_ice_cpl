...
CONTAINS
   ...
   SUBROUTINE lim_dyn_init
      ...
      NAMELIST/namicedyn/ nn_limadv, nn_limadv_ord,  &
         &                nn_icestr, ln_icestr_bvf, ln_icestr_fl, rn_icestr_c1, rn_icestr_c2, rn_pe_rdg, rn_pstar, rn_crhg, rn_cio,&
         & rn_creepl, rn_ecc, nn_nevp, rn_relast, ln_landfast, rn_gamma, rn_icebfr, rn_lfrelax              
      !!-------------------------------------------------------------------
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namicedyn in configuration namelist', lwp )
      ...
      IF(lwp) THEN                        ! control print
         ...
         ! Guillaume B. -> In the end I did not use that, not enough time to investigate
         WRITE(numout,*)'    Including floe size in ice strength comp.                   ln_icestr_fl  = ', ln_icestr_fl
         WRITE(numout,*)'    Including floe size in ice strength comp.  param 1          rn_icestr_c1  = ', rn_icestr_c1
         WRITE(numout,*)'    Including floe size in ice strength comp.  param 2          rn_icestr_c2  = ', rn_icestr_c2
         ! End Guillaume B.
