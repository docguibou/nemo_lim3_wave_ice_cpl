...
CONTAINS
   ...
   SUBROUTINE sbc_ice_lim( kt, ksbc )
      ...
      !!
      INTEGER  ::   jl, ji, jj, jf                 ! dummy loop index
...
# endif
! GB : When ln_limthd= FALSE, no call to lim_update2=no call zapsmall
...
!!gm   remark, the ocean-ice stress is not saved in ice diag call above .....  find a solution!!!
   ...
   SUBROUTINE lim_run_init
      ...
      NAMELIST/namicerun/ jpl,jpf, nlay_i, nlay_s, rn_amax_n, rn_amax_s, cn_icerst_in, cn_icerst_indir,   &
...
904   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namicediag in configuration namelist', lwp )
      ...
      IF(lwp) THEN                        ! control print
         ...
         WRITE(numout,*) '   number of ice floes categories                          = ', jpf
         ...
         WRITE(numout,*) '       0: advection only (as 1 + prescribed velocity, bypass rheology)'
         WRITE(numout,*) '     Ice floes redistribution switch          nn_limfldis  = ', nn_limfldis
         WRITE(numout,*) '       2: Redistribution on the principle of Zhang et al. (2015) with beta following a power-law'
         WRITE(numout,*) '       1: only categories >dmax contribute, with q=1'
         WRITE(numout,*) '       0: equirepartition in each category'
         WRITE(numout,*) '     Ice floes redistribution coeff                rn_flq  = ', rn_flq
         WRITE(numout,*)
   ...
   SUBROUTINE lim_itd_init
      ...
      !!
      !! ** Purpose :   Initializes the ice thickness distribution & floe size distribution
      !! ** Method  :   ...
      !! ** input   :   Namelist namiceitd
      !!-------------------------------------------------------------------
      INTEGER  ::   ios                 ! Local integer output status for namelist read
      NAMELIST/namiceitd/ nn_catbnd, rn_himean, nn_catfbnd, rn_catfthresh, rn_fsdmax, rn_fsdmin, rn_fsdwidth, rn_maxpareto
      !
      INTEGER  ::   jl,jf                   ! dummy loop index
      REAL(wp) ::   zc1, zc2, zc3, zx1  ! local scalars
      REAL(wp) ::   zhmax, znum, zden, zalpha, zfrag, zgamma, z_area !
      !!------------------------------------------------------------------
      !
...
906   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namiceitd in configuration namelist', lwp )
      ...
      IF(lwp) THEN                        ! control print
         ...
         WRITE(numout,*) '   shape of floe size categories distribution                    nn_catfbnd    = ', nn_catfbnd
         WRITE(numout,*) '   max. floe size                                                rn_fsdmax     = ', rn_fsdmax
         WRITE(numout,*) '   min. floe size                                                rn_fsdmin     = ', rn_fsdmin
         WRITE(numout,*) '   witdh of floe size cat. (if nn_catfbnd=1)                     rn_fsdwidth   = ', rn_fsdwidth
         WRITE(numout,*) '   coefficient for local dmax determination                      rn_catfthresh = ', rn_catfthresh
      ...
      di_mean(:)   = 0._wp
      ! Allocate values
      di_max(0)=rn_fsdmin          !d_min, may be passed as namelist parameter         
      ! Maybe it should be it only defined if ln_wavice is true ?
      SELECT CASE ( nn_catfbnd  )    ! type of ice floes categories distribution
      
      CASE (1)
         DO jf = 1, jpf-1                
            di_max(jf)   =  rn_fsdmin +rn_fsdwidth*jf 
         END DO
      CASE (2)
         z_area       = 1._wp
         DO jf = 1, jpf-1
            z_area       =      z_area - 1._wp /jpf          
            di_max(jf)   =  (rn_maxpareto**(-zgamma+2.) * (1._wp-z_area)  + z_area * rn_fsdmin**(-zgamma+2.))**(1./(-zgamma+2.) ) 
         END DO
      END SELECT

      DO jf = 1, jpf-1
         di_mean(jf)     =  ( di_max(jf-1)+ di_max(jf) ) /2._wp
         di_frac(jf)=( di_max(jf)**(-zgamma+2.) - di_max(jf-1)**(-zgamma+2.) ) / ( di_max(jf)**(-zgamma+2.) - rn_fsdmin**(-zgamma+2.) )
      END DO
      di_max(jpf)  = rn_fsdmax
      di_mean(jpf) = rn_fsdmax
      di_frac(jpf) = ( di_max(jf)**(-zgamma+2) - di_max(jf-1)**(-zgamma+2) ) / &
      & ( di_max(jf)**(-zgamma+2) - rn_fsdmin**(-zgamma+2) ) ! fraction associated to the highest category in case di_max(jf) = dmax



      IF(lwp) WRITE(numout,*) '         di_max ', di_max(0:jpf)
      IF(lwp) WRITE(numout,*) '         di_mean', di_mean(jpf) 
      IF(lwp) WRITE(numout,*) '         di_frac', di_frac(jpf) 
      !
