...
!
      INTEGER  ::   ji, jj, jk, jm, jl, jt, jf  ! dummy loop indices
...
!
      ...
      REAL(wp) ::    zdv, zda
      REAL(wp), POINTER, DIMENSION(:,:)      ::   zatold, zeiold, zesold, zsmvold, zaiflt  
      REAL(wp), POINTER, DIMENSION(:,:,:)    ::   zhimax, zviold, zvsold, zaiflold
! --- diffusion --- !
...
! --- prather only --- !
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:)    ::   z0afl
      REAL(wp), POINTER, DIMENSION(:,:,:,:)  ::   z0ei
!!
!!---------------------------------------------------------------------
      IF( nn_timing == 1 )  CALL timing_start('limtrp')

      CALL wrk_alloc( jpi,jpj,                            zatold, zeiold, zesold, zsmvold, zaiflt )
      CALL wrk_alloc( jpi,jpj,jpl,                        zhimax, zviold, zvsold )
      CALL wrk_alloc( jpi,jpj,jpf,                        zaiflold )
...
! TEST GB
         DO jj = 1, jpj
            DO ji = 1, jpi
               IF ( ABS( zaiflt(ji,jj)-at_i(ji,jj) ) > epsi06 ) WRITE(*,*) 'CONC. NOT CONSERVED', 'i :', ji, 'j :', jj, 't :' , kt, 'diff ;', ABS( zaiflt(ji,jj)-at_i(ji,jj) ), zaiflt(ji,jj), at_i(ji,jj)
            END DO
         END DO
...
! advection
         DO jt = 1, initad
            ...
            END DO
            DO jf = 1, jpf
               CALL lim_adv_umx( kt, zdt, zudy, zvdx, zcu_box, zcv_box, afl_i(:,:,jf) )   ! Ice  floes
            END DO
         END DO
!
...
!=============================!
         ...
         CALL wrk_alloc( jpi,jpj,jpf,        z0afl )
...
!-------------------------
         ...
         END DO
         DO jf = 1, jpf
            z0afl(:,:,jf)   = afl_i (:,:,jf) * e1e2t(:,:)    ! Floe content
         END DO
         ...
         IF( MOD( ( kt - 1) / nn_fsbc , 2 ) == 0 ) THEN       !==  odd ice time step:  adv_x then adv_y  ==!
            DO jt = 1, initad
               ...
               END DO
               DO jf = 1, jpf
                  CALL lim_adv_x( zusnit, u_ice, 1._wp, zarea, z0afl  (:,:,jf), sxfl(:,:,jf),   &    !--- ice floes      ---
                     &                                       sxxfl(:,:,jf), syfl(:,:,jf), syyfl(:,:,jf), sxyfl(:,:,jf)  )

                  CALL lim_adv_y( zusnit, v_ice, 0._wp, zarea, z0afl  (:,:,jf), sxfl(:,:,jf),   &
                     &                                       sxxfl(:,:,jf), syfl(:,:,jf), syyfl(:,:,jf), sxyfl(:,:,jf)  )
               END DO
            END DO
         ELSE
            DO jt = 1, initad
               ...
               END DO
               DO jf = 1, jpf
                  CALL lim_adv_y( zusnit, v_ice, 1._wp, zarea, z0afl  (:,:,jf), sxfl(:,:,jf),   &   !--- ice floes      ---
                     &                                       sxxfl (:,:,jf), syfl(:,:,jf), syyfl(:,:,jf), sxyfl(:,:,jf)  )

                  CALL lim_adv_x( zusnit, u_ice, 0._wp, zarea, z0afl  (:,:,jf), sxfl(:,:,jf),   &
                     &                                       sxxfl(:,:,jf), syfl(:,:,jf), syyfl(:,:,jf), sxyfl(:,:,jf)  )
               END DO
            END DO
         ENDIF
...
!-------------------------------------------
         ...
         CALL wrk_dealloc( jpi,jpj,jpf,        z0afl )
...
! --- Account for conc. correction (at_i) in floes categories conc. (afl_i) -----------------
      zaiflt(:,:) = SUM( afl_i(:,:,:), dim=3 )
      DO jf = 1, jpf
        DO jj = 1, jpj
          DO ji = 1, jpi
         ...
         END DO
      END DO
! --- In case (at_i) has increased from 0, add (afl_i) -----------------
      DO jj = 1, jpj
        DO ji = 1, jpi
            IF ( ( ABS( zaiflt(ji,jj)-at_i(ji,jj) ) > epsi06).AND.(zaiflt(ji,jj)==0) ) WRITE(*,*) "/!\\ You did something ugly to avoid : at_i >0 and afl=0"
            IF ( ( ABS( zaiflt(ji,jj)-at_i(ji,jj) ) > epsi06).AND.(zaiflt(ji,jj)==0) )   afl_i(ji,jj,jpf) = at_i(ji,jj)
        END DO
      END DO
...
! TEST GB 2
      ...
      DO jj = 1, jpj
        DO ji = 1, jpi
          IF ( ABS( zaiflt(ji,jj)-at_i(ji,jj) ) > epsi06 ) WRITE(*,*) 'NOT CONSERVED 2 ', 'i :', ji, 'j :', jj, 't :' , kt, 'diff ;', ABS( zaiflt(ji,jj)-at_i(ji,jj) ),'at_i(ji,jj)  '  ,at_i(ji,jj),'zaiflt(ji,jj)   ',zaiflt(ji,jj)
        END DO
      END DO
