...
!                                     !!** ice-thickness and ice floes distribution namelist (namiceitd) **
...
!!--------------------------------------------------------------------------
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:)   ::   di_c           !: Center of floe size categories in floe diameters space
...
! * Ice floes distribution variables
      WRITE(*,*)
      ii = ii + 1
      ALLOCATE( di_max(0:jpf),di_c(jpf), di_mean(jpf), di_frac(jpf), STAT=ierr(ii) )
