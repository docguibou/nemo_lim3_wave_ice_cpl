...
! Sum of external forces (explicit solution) = F + tau_ia + Coriolis + spg + tau_io
                  zTauE = zfU(ji,jj) + zTauU_ia(ji,jj) + zTauU_iw(ji,jj) +  + zCor + zspgU(ji,jj) + zTauO * ( u_oce(ji,jj) - u_ice(ji,jj) )
!    IF ((ji.EQ.50).AND.(jj.EQ.50)) WRITE(*,*) 'TAUE',zTauE, 'TAIWX',TAIWX(ji,jj), "zCor",zCor, 'zTauO' ,zTauO,'zfU(ji,jj)',zfU(ji,jj)
!   IF ((ji.EQ.50).AND.(jj.EQ.50)) WRITE(*,*) 'Rapport   :' ,TAIWX(ji,jj)/zfU(ji,jj)
! landfast switch => 0 = static friction ; 1 = sliding friction
                  rswitch = 1._wp - MIN( 1._wp, ABS( SIGN( 1._wp, ztauE - tau_icebfr(ji,jj) ) - SIGN( 1._wp, zTauE ) ) )
