...
!
      ...
      DO jk = 1, nlay_s       ! Old snow temperature
         ...
         END DO
      END DO
! WRITE(*,*) "Temp diff : Init" , t_i_1d(30,:)
...
!!case of only one layer in the ice (ice equation is altered)
               ...
               ENDIF
!WRITE(*,*) "zindterm 4 :", zindterm(30,:),"tbo ", t_bo_1d(30) , zkappa_i(30,1)
               IF ( t_su_1d(ji) < rt0 ) THEN
!IF (ji==30) WRITE(*,*) "case 1"
!------------------------------------------------------------------------------|
...
!IF (ji==30) WRITE(*,*) "zindterm 5 :",zindterm(30,:)!ztrid(30,1,1), ztrid(30,1,2), ztrid(30,1,3), zindterm(30,1), ztrid(30,2,1), ztrid(30,2,2), ztrid(30,2,3),zindterm(ji,2)
...
!!case of only one layer in the ice (surface & ice equations are altered)
                  ...
                  ENDIF
!     IF (ji==30) WRITE(*,*)  "zindterm :" , zindterm(30,numeqmax(30)),"zindtib:",ztib(ji,1),zeta_i(ji,1), ( zradab_i(ji,1) + zkappa_i(ji,1) * t_bo_1d(ji) ),t_su_1d(ji) * zeta_i(ji,1) * zkappa_i(ji,0) * 2.0
               ENDIF
            ENDIF
...
! McGraw-Hill 1984.
         ...
         END DO
!     WRITE(*,*)  "zindtbis 1 :" , zindtbis(30,numeqmax(30))
...
!   IF (ji==30) WRITE(*,*)  "COUCOU :" , numeq,  zindterm(ji,numeq), - ztrid(ji,numeq,1) * zindtbis(ji,numeq-1), zdiagbis(ji,numeq-1)
            END DO
         END DO
!    WRITE(*,*)  "zindtbis 2 :" , zindtbis(30,numeqmax(30))

         DO ji = kideb , kiut
! ice temperatures
            t_i_1d(ji,nlay_i)    =  zindtbis(ji,numeqmax(ji)) / zdiagbis(ji,numeqmax(ji))
         END DO
!   WRITE(*,*) "Temp diff : 1" , t_i_1d(30,:), "num :" , zindtbis(30,numeqmax(30))," dem ", zdiagbis(30,numeqmax(30))
