...
!!--------------------------------------------------------------------
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:) :: zh_i_ini, za_i_ini, z_afl_i_ini        !data by cattegories to fill
      INTEGER , POINTER, DIMENSION(:)     :: itest
!--------------------------------------------------------------------

      CALL wrk_alloc( jpi, jpj, jpl, zh_i_ini,  za_i_ini )
      CALL wrk_alloc( jpi, jpj, jpf, z_afl_i_ini )
...
!
                     ...
                     z_afl_i_ini(ji,jj,jpf)=zat_i_ini(ji,jj) ! Only the last category is filled -> floes are supposed to be initially unbroken.
...
! ice volume, salt content, age content
         ...
         DO jf = 1, jpf ! loop over FSD categories
            DO jj = 1, jpj
               DO ji = 1, jpi
                 afl_i(ji,jj,jf)   = zswitch(ji,jj) * z_afl_i_ini(ji,jj,jf)
               END DO
            END DO
         END DO
