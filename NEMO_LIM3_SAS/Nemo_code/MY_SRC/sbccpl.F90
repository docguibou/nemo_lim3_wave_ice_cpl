MODULE sbccpl
   ...
   INTEGER, PARAMETER ::   jpr_taiwx  = 54
   INTEGER, PARAMETER ::   jpr_taiwy  = 55
   INTEGER, PARAMETER ::   jpr_icef   = 56
   ...
   INTEGER, PARAMETER ::   jps_icef   = 33   ! ice floe diameter sent back to the wave model 
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: sbccpl.F90 7815 2017-03-20 17:31:43Z clem $
...
CONTAINS
   ...
   SUBROUTINE sbc_cpl_init( k_ice )     
      ...
      NAMELIST/namsbc_cpl/  sn_snd_temp , sn_snd_alb  , sn_snd_thick , sn_snd_crt   , sn_snd_co2,      & 
         ...
         &                  sn_rcv_icb , sn_rcv_isf, sn_rcv_taiwx, sn_rcv_taiwy, sn_rcv_icef       ,   &
         &                  sn_snd_icef                                                                
...
902   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namsbc_cpl in configuration namelist', lwp )
      ...
      IF( TRIM( sn_rcv_dqnsdt%cldes ) == 'none' .AND. k_ice /= 0 .AND. k_ice /= 4 .AND. nn_components /= jp_iam_sas .AND. ln_usr == .FALSE. ) &
...
!received new variable tauice from WW3
      IF( srcv(jpr_taiwx)%laction     ) THEN ; TAIWX(:,:)    = frcv(jpr_taiwx)%z3(:,:,1)
      ELSE                                   ; TAIWX(:,:)=0._wp
      ENDIF
      IF( srcv(jpr_taiwy)%laction     ) THEN ; TAIWY(:,:)    = frcv(jpr_taiwy)%z3(:,:,1)
      ELSE                                   ; TAIWY(:,:)=0._wp
      ENDIF
      IF( srcv(jpr_icef)%laction      ) THEN ; ICEF_r(:,:)   = frcv(jpr_icef)%z3(:,:,1)
      ELSE                                   ; ICEF_r(:,:)=1000._wp
      ENDIF
...
!         ENDIF 
      ...
      IF( ssnd(jps_icef)%laction ) THEN 
         CALL cpl_snd( jps_icef, isec, RESHAPE ( dmaxfl_i, (/jpi,jpj,1/) ), info ) 
      END IF 
      !                                                      ! ------------------------- ! 
