...
CONTAINS
   ...
   SUBROUTINE lim_var_glo2eqv
      ...
      ! Small correction of the FSD without changing its shape
      ! It is necessary as at_i is modified in lot of places without accounting
      ! for FSD
      DO jj = 1, jpj
         DO ji = 1, jpi
            IF (zaiflt(ji,jj)/=at_i(ji,jj)) THEN 
               ...
               ELSE
                 DO jf = 1, jpf
                   afl_i(ji,jj,jf) = afl_i(ji,jj,jf)*at_i(ji,jj)/MAX( zaiflt(ji,jj), epsi20 ) * rswitch
                 END DO
               END IF
            END IF
         END DO
      END DO
      ! End of afl_i correction
      !     
      !--------------------
      ! Setting D_max 
      !--------------------
      DO ji = 1 , jpi 
         DO jj = 1 , jpj
            ! 1.) Reinitialize dmax everywhere at_i>0.
            rswitch       = MAX( 0._wp , SIGN( 1._wp, at_i(ji,jj ) - epsi10 ) )
            dmaxfl_i(ji,jj)=rn_fsdmax * rswitch   
            ! 2.) Follow D_max definition from Boutin et al. (2019)
            ! Is ice broken or not ?    
            IF (afl_i(ji,jj,jpf)<MAX(0.5* at_i(ji,jj),di_frac(jf) *rn_catfthresh * at_i(ji,jj))) THEN
               ! If ice is broken, then compare it to a power-law distribution
               DO jf=jpf-1, 1, -1    
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            IF (ABS( zaiflt(ji,jj)-at_i(ji,jj) ) > epsi06 ) THEN
               WRITE(*,*) 'lim_var_glo2eqv -> sum FSD different from a_i'
               WRITE(*,*)  'i index :', ji, '; j index :', jj, '; diff =',  zaiflt(ji,jj)-at_i(ji,jj) 
            END IF
         END DO
      END DO
      ICEF_s=dmaxfl_i 
...
! Floes cat.
   ...
   SUBROUTINE lim_var_itd( zhti, zhts, zai, zht_i, zht_s, za_i )
    ...
    SUBROUTINE lim_var_fsd
      ...
      !!                ***  ROUTINE lim_var_fsd   ***
      !!
      !! ** Purpose :  Redistribute the floe size distribution after wave break-up
      !!
      !!                  
      !!
      !! ** Method:   
      !!                
      !!               1) Determine if break-up has occured by comparing received Dmax to local Dmax
      !!
      !!               2) If break-up has occured, update Dmax and redistribute the FSD
      !!              
      !!               3) If not, do nothing
      !!
      !!
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:)    ::   zafl_old            ! Old FSD -> used for redistribution
      ! Array for FSD redistribution after break-up (Zhang et al., 2015)
      REAL(wp), POINTER, DIMENSION(:,:)      ::   zbeta_array 
      REAL(wp), POINTER, DIMENSION(:)        ::   zq_array   

      INTEGER  :: i_break 
      REAL(wp) ::  zfrag, zgamma, zdmin, zdelta
      ...
      !--------------------------------------------------------------------
      i_break          = 1
      zfrag            = 0.9                                ! fragility (see Toyota et al. 2011)
      zgamma           = 2._wp + log(zfrag)/log(2._wp)      ! Power-law coefficient
      zdmin            = rn_fsdmin                          ! namelist parameter ?
      zafl_old(:,:,:)  = afl_i(:,:,:)

      !--------------------------------------------------------------------
      ! FSD redistribution after break-up following Boutin et al. 2019
      !--------------------------------------------------------------------
      DO ji = 1, jpi 
         DO jj = 1, jpj
            ! Receive D_max from the wave model
            IF ((at_i(ji,jj)>0._wp).AND.(ICEF_r(ji,jj)>0._wp)) dmaxfl_i(ji,jj)=ICEF_r(ji,jj) 

            DO jf = 1, jpf-1
               ! Has break-up occured ?
               ! If Dmax from wave model is lower than di_max associated with the cat. jf -> yes
               IF ( ( dmaxfl_i(ji,jj)< di_max(jf) ).AND.(at_i(ji,jj)>0._wp) ) THEN 
                  i_break=jf
                  dmaxfl_i(ji,jj)=di_max(i_break) ! This is the new dmax

                  !! nn_limfldis <2 does not work actually
                  !! FSD must be redistributed  (see Boutin et al., 2019)
                  IF (nn_limfldis==2) THEN
                     ...
                     DO jfn = jpf,1,-1
                        zdelta =  - zafl_old(ji,jj,jfn)+( di_max(jfn)**(-zgamma+2) - di_max(jfn-1)**(-zgamma+2) ) / ( di_max(i_break)**(-zgamma+2) - zdmin**(-zgamma+2) ) * SUM(zafl_old(ji,jj,:))
                        DO jfm = 1, jpf
                        ...
                        ! The namelist fn_flq has only be validated for rn_flq = -1
                        !                         ! 
                        ...
                        !      END IF
                           END IF
                        END IF ! jfn>i_break
                        afl_i(ji,jj,jfn) = zafl_old(ji,jj,jfn) * (1._wp -zq_array(jfn)) +  SUM(zbeta_array(:,jfn)*zafl_old(ji,jj,:)*zq_array(:))  
                     ENDDO     ! jfn
 
                     EXIT      ! do jf  
                  END IF       ! if nn_limfldis=2         
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            IF ( ABS( SUM(afl_i(ji,jj,:))- at_i(ji,jj)) > epsi06 ) THEN
               WRITE(*,*) 'lim_var_fsd(): FSD conservation problem: ', 'i index :', ji, '; j index :', jj, '; diff ;', ABS( SUM(afl_i(ji,jj,:))-at_i(ji,jj) )
               WRITE(*,*) "at_i_old", SUM( zafl_old(ji,jj,:)),'; at_i_new :',SUM(afl_i(ji,jj,:)) ,'; at_i :',at_i(ji,jj) 
               WRITE(*,*) 'vecteur zafl_old :',zafl_old(ji,jj,:),'; vecteur zafl_new :' ,afl_i(ji,jj,:),' ;d =', dmaxfl_i(ji,jj),ICEF_r(ji,jj)
            END IF
         END DO
      END DO
      !  END CHECK
